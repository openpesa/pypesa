MPESA_PUBLIC_KEY = ''

MPESA_API_KEY = ''

MPESA_BASE_URL = 'openapi.m-pesa.com'
MPESA_GET_SESSION_URL = '/sandbox/ipg/v2/vodacomTZN/getSession/'
MPESA_C2BPAYMENT_URL = '/sandbox/ipg/v2/vodacomTZN/c2bPayment/singleStage/'
MPESA_REVERSAL_URL = '/sandbox/ipg/v2/vodacomTZN/reversal/'
MPESA_B2CPAYMENT_URL = '/sandbox/ipg/v2/vodacomTZN/b2cPayment/'
MPESA_B2BPAYMENT_URL = '/sandbox/ipg/v2/vodacomTZN/b2bPayment/'
MPESA_TRANSACTION_STATUS_URL = '/sandbox/ipg/v2/vodacomTZN/queryTransactionStatus/'
MPESA_DIRECT_DEBIT_CREATE_URL = '/sandbox/ipg/v2/vodacomTZN/directDebitCreation/'
MPESA_DIRECT_DEBIT_PAYMENT_URL = '/sandbox/ipg/v2/vodacomTZN/directDebitPayment/'
